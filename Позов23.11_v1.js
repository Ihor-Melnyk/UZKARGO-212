...
function onTaskExecuteShtraf(routeStage) {
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("SumaSud12_rah").value) throw 'Помилка! Неможливо виконати завдання не заповнивши  поля таблиць "Сума за рішенням суду" та "Судові витрати"';
  }
}
...
function setSecondTableCall(elName, fromCall, toCall) {
  ...
  if (el) {
    ...
    if (fromCall == "typeInvoice")
      buf = parentTable
        .filter((x) => {
          return x.find((y) => y.value == el);
        })
        ?.map((s) => s.find((n) => n.code == fromCall)?.text)[0];
    if (buf) {
...
function setUnitForMask() {
  ...
  if (!UnitForMask) {
    ...
    if (Responsible) EdocsApi.setAttributeValue({ code: "UnitForMask", value: EdocsApi.getEmployeeDataByEmployeeID(Responsible).unitCode, text: null });
  }
}
...
function onTaskExecuteShtraf(routeStage) {
  if (routeStage.executionResult != "Shtraf") {
    ...
    if (Invoice_Claim_table2 && Invoice_Claim_table2.length > 0) {
      Invoice_Claim_table2.map((x) => x.find((y) => y.code == "Invoice_Claim_number2").value).every((s) => {
        if (s == null) {
          buf = true;
          str += '"№  накладної" таблиці "Документи, що додані до позову", ';
        }
      });
      Invoice_Claim_table2.map((x) => x.find((y) => y.code == "Invoice_Claim_Amaunt2").value).every((s) => {
        if (s == null) {
          buf = true;
          str += '"Сума" таблиці "Документи, що додані до позову", ';
        }
      });
      Invoice_Claim_table2.map((x) => x.find((y) => y.code == "SpravaBula_Ni").value).every((s) => {
        if (s == null) {
          buf = true;
          str += '"Була претензійна справа" таблиці "Документи, що додані до позову", ';
        }
      });
    } else {
      buf = true;
...
function onTaskExecuteAssignResp(routeStage) {
  if (routeStage.executionResult != "rejected") {
    ...
    if (!EdocsApi.getAttributeValue("RequirementType").value) {
      throw `Заповніть поле(я) "Тип вимог"`;
    }
  }
}
...
function AssignRespTask() {
  debugger;
  var stateTask = EdocsApi.getCaseTaskDataByCode("AssignResp")?.state;
...
function onSearchRequirementType(searchRequest) {
  searchRequest.filterCollection.push({
    attributeCode: "TypeDoc",
    value: "Позов",
  });
}
